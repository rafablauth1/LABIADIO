            ...
            <tbody className="divide-y divide-white/5">
              {items.map((p: any) => (
                <tr key={p.id} className="hover:bg-white/3 transition-colors">
                  ...
                  <td className="px-4 py-2.5 text-white/50 text-[10px] max-w-[140px]">
                    {(p.normas || []).join(', ') || '—'}
                  </td>
              ...
              {items.length === 0 && (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-white/25 italic text-sm">Nenhum procedimento cadastrado ainda.</td></tr>
              )}
            </tbody>
          </table>
        </div>
