'use client'

import { useState, useEffect } from 'react'
import { Plus } from 'lucide-react'
import { createClient } from '@/lib/supabase/client'
import AuxiliarModal from '@/components/modals/AuxiliarModal'

function fmt(d: string | null) {
  if (!d) return '—'
  const s = d.slice(0, 10)
  return s.slice(8, 10) + '/' + s.slice(5, 7) + '/' + s.slice(0, 4)
}

export default function AuxiliaresPage() {
  const supabase = createClient()
  const [items, setItems] = useState<any[]>([])
  const [open, setOpen] = useState(false)

  async function load() {
    const { data } = await supabase.from('auxiliares').select('*').order('tag')
    setItems(data || [])
  }

  useEffect(() => { load() }, [])

  return (
    <div>
      <div className="flex items-start justify-between mb-6">
        <div>
          <p className="font-mono text-[9px] tracking-[2.5px] text-gold uppercase mb-1">Equipamentos</p>
          <h1 className="font-display font-bold text-2xl text-white">Aparelhos Auxiliares</h1>
          <p className="text-white/40 text-sm mt-1">Suporte ao Ensaio</p>
        </div>
        <button className="btn-primary text-xs" onClick={() => setOpen(true)}>
          <Plus size={13} /> Novo
        </button>
      </div>

      <div className="card overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-[11.5px]">
            <thead>
              <tr className="border-b border-white/7 bg-navy">
                {['TAG','DESCRIÇÃO','CATEGORIA','VINCULADO A','MANUTENÇÃO','STATUS',''].map(h => (
                  <th key={h} className="px-4 py-2.5 text-left font-mono text-[8px] tracking-[1.8px] text-white/35 uppercase">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {items.map((a: any) => (
                <tr key={a.id} className="hover:bg-white/3 transition-colors">
                  <td className="px-4 py-2.5"><span className="tag-chip">{a.tag}</span></td>
                  <td className="px-4 py-2.5 text-white/80 max-w-[200px] truncate">{a.descricao}</td>
                  <td className="px-4 py-2.5 text-white/50 text-[10px]">{a.categoria}</td>
                  <td className="px-4 py-2.5"><span className="tag-chip">{a.vinculado || '—'}</span></td>
                  <td className="px-4 py-2.5 font-mono text-[10px] text-white/50">{fmt(a.manut)}</td>
                  <td className="px-4 py-2.5"><span className="badge-success text-[9px]">Ativo</span></td>
                  <td className="px-4 py-2.5"><button className="text-white/25 hover:text-teal transition-colors font-mono text-[10px]">Ver →</button></td>
                </tr>
              ))}
              {items.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center text-white/25 italic text-sm">
                    Nenhum aparelho auxiliar cadastrado ainda.
                  </td>
                </tr>
              )}
            </tbody>
          </table>
        </div>
      </div>

      <AuxiliarModal open={open} onClose={() => { setOpen(false); load() }} />
    </div>
  )
}
